"""
run_node.py — Nautilus BacktestNode for LiveStrategy
=====================================================
ใช้ TradeTick (aggTrades) → Nautilus aggregate เป็น ValueBar ($50k) อัตโนมัติ
เหมือน live_engine ที่ใช้ VolumeBarAggregator($50k) ทุกประการ
...
ต้องรัน fetch_data.py ก่อน:
    python fetch_data.py --days 30       # ดึง TradeTick + Bar
"""
...
# ValueBar $50k — เหมือน live_engine VolumeBarAggregator(threshold_usd=50_000)
...
def make_config(
    *,
    ...
    return BacktestRunConfig(
        ...
        data=[
            # TradeTick → Nautilus aggregate เป็น ValueBar INTERNAL อัตโนมัติ
            BacktestDataConfig(
...
def make_quick_sweep() -> list[BacktestRunConfig]:
    ...
    return [
        ...
        make_config(atr_sl_multiplier=2.0, atr_tp_multiplier=6.0, run_id="RR-3:1"),
...
def make_full_sweep() -> list[BacktestRunConfig]:
    ...
    for sl, tp, name in [
        (1.5, 3.0, "SL1.5-TP3"), (2.0, 6.0, "SL2-TP6"),
        (2.5, 5.0, "SL2.5-TP5"), (3.0, 6.0, "SL3-TP6"), (1.0, 3.0, "SL1-TP3"),
    ]:
        configs.append(make_config(atr_sl_multiplier=sl, atr_tp_multiplier=tp,
                                   run_id=f"ATR-{name}"))
    ...
    for lmin, lmax, smin, smax, name in [
        ...
        configs.append(make_config(rsi_long_min=lmin, rsi_long_max=lmax,
    ...
    for fast, med, trend, name in [
        ...
        configs.append(make_config(ema_fast=fast, ema_medium=med, ema_trend=trend, run_id=name))

    for ml, name in [(3, "CB-STRICT"), (5, "CB-DEFAULT"), (8, "CB-LOOSE")]:
        configs.append(make_config(max_consecutive_losses=ml, run_id=name))
...
def run():
    ...
    # ตรวจว่ามี TradeTick ใน catalog
    try:
    ...
    if tick_count == 0:
        print("[ERROR] No TradeTick data found!")
        print("        Run: python fetch_data.py --days 30")
        return

    W = 72
    print("=" * W)
    mode_label = "FULL SWEEP" if FULL_SWEEP else ("SWEEP" if SWEEP_MODE else "SINGLE")
    print(f"  LiveStrategy Backtest — {mode_label}".center(W))
    print("=" * W)
    print(f"  Catalog   : {CATALOG_PATH.resolve()}")
    print(f"  Pair      : {instruments[0].id}")
    print(f"  TradeTick : {tick_count:,} records")
    print(f"  Bar type  : ValueBar $50k (INTERNAL aggregation)")
